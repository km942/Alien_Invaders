...
Eben Hill emh238, Kiyam Merali km942
12/03/2023
"""
from consts import *
from game2d import *
...
class Invaders(GameApp):
    ...
    #
    # Attribute _truth: You know the truth
    # Invariant: _truth is a boolean
    #
    # Attribute _image: The image that shows at the end of the game
    ...
    def start(self):
        ...
        if STATE_INACTIVE==self._state:
            self._text=GLabel(x=(GAME_WIDTH/2),y=(GAME_HEIGHT/2),\
            text="Press 'S' to start",font_name="RetroGame.ttf")
        else:
            self._text=None
        self._wave=None
        self._truth=False
    ...
    def enforce_pause(self):
        """
        Pauses the game (after death of ship) until 'r' is pressed
        """
        if self._wave.getDead()==True:
            self._state = STATE_PAUSED
            self._text=GLabel(x=(GAME_WIDTH/2),y=(GAME_HEIGHT/2),\
            text="You Died. Press 'R' to Continue",font_name="RetroGame.ttf")
        if self._state == STATE_PAUSED:
            if self.input.is_key_pressed('r'):
                self._wave.setDead(False)
                self._wave.respawn_ship()
    ...
    def check_endgame(self):
        """
        Checks for victory or defeat and delivers it if necessary
        """
        if self._wave.assert_win_conditions():
            self.declare_victory()
        if self._wave.assert_lose_conditions():
            self.declare_loss()

    def declare_victory(self):
        """
        Ends the game with a victory screen
        """
        self._state = STATE_COMPLETE
        self._text=GLabel(x=(GAME_WIDTH/2),y=(GAME_HEIGHT/2),\
        text="Congratulations, you won!",font_name="RetroGame.ttf")
        self._image=GImage(height=GAME_HEIGHT-20,x=GAME_WIDTH/2,\
        y=GAME_HEIGHT/2,source="win.jpg")

    def declare_loss(self):
        """
        Ends the game with a loss screen
        """
        self._state = STATE_COMPLETE
        self._text=GLabel(x=(GAME_WIDTH/2),y=(GAME_HEIGHT/2),\
        text="You failed",font_name="RetroGame.ttf")
        self._image=GImage(width=GAME_WIDTH-20,x=GAME_WIDTH/2,\
        y=GAME_HEIGHT/2,source="lose.jpg")
